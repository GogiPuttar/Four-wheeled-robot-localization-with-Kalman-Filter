...
# Calculate mean
def getNewMeanOD(currx, currz):

    global meanOD
    
    meanOD[0] = ((ctr - 1) / ctr) * meanOD[0] + currx/ ctr
    
    meanOD[1] = ((ctr - 1) / ctr) * meanOD[0] + currz / ctr
    
# Calculate Variance
def getNewVarianceOD(currx, currz, gpscoords):

    global meanOD, varianceOD
    
    varianceOD[0] = ((ctr - 1) / ctr) * varianceOD[0] + ((currx - meanOD[0])**2) / ctr    
    
    varianceOD[1] = ((ctr - 1) / ctr) * varianceOD[1] + ((currz - meanOD[1])**2) / ctr
...
while robot.step(timestep) != -1:
    ...
    # Mean and variance
    getNewMeanOD(currx - gpscoords[0], currz - gpscoords[2])
    getNewVarianceOD(currx, currz, gpscoords)
    
    print(str(round(meanOD[0], 5)) + "\t" + str(round(meanOD[1], 5)) + "\t" + str(round(varianceOD[0], 5)) + "\t" + str(round(varianceOD[1], 5)))
